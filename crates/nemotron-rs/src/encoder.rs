...
unsafe fn causal_pad(ctx: Context, mut x: Tensor) -> Tensor {
    let ty = (*x).type_;
    let left = sys::ggml_fill(
        ctx,
        sys::ggml_new_tensor_4d(ctx, ty, 2, (*x).ne[1], (*x).ne[2], (*x).ne[3]),
        0.0,
    );
    ...
    let top = sys::ggml_fill(
        ctx,
        sys::ggml_new_tensor_4d(ctx, ty, (*x).ne[0], 2, (*x).ne[2], (*x).ne[3]),
        0.0,
    );
...
pub(crate) unsafe fn build_encoder(ctx: Context, model: &Model, mel: Tensor) -> EncoderBuild {
    ...
    let joined = sys::ggml_concat(ctx, x, prompt, 0);
    let mut h = sys::ggml_mul_mat(ctx, weight(model, "prompt.mlp.0.weight"), joined);
    h = sys::ggml_relu(ctx, sys::ggml_add(ctx, h, weight(model, "prompt.mlp.0.bias")));
    let out = sys::ggml_add(
        ctx,
        sys::ggml_mul_mat(ctx, weight(model, "prompt.mlp.2.weight"), h),
        weight(model, "prompt.mlp.2.bias"),
    );
    EncoderBuild {
        output: out,
        position,
...
pub(crate) fn chunk_mask(time: usize, left: usize, right: usize) -> Vec<f32> {
    let chunk = right + 1;
    let left_chunks = left / chunk;
    let mut result = vec![f32::NEG_INFINITY; time * time];
    for q in 0..time {
        let qc = q / chunk;
        let min = qc.saturating_sub(left_chunks) * chunk;
        let max = ((qc + 1) * chunk).min(time);
        for k in min..max {
            result[q * time + k] = 0.0;
        }
    }
